        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold text-sm uppercase tracking-widest">
              AI Optimizer: High-Fidelity Review
            </h2>
            <p className="text-[10px] text-indigo-200 font-medium">
              Review and accept ATS-optimized enhancements before applying.
            </p>
          </div>
        </div>
      ...
      <div className="flex-grow overflow-y-auto p-6 md:p-12 font-serif bg-slate-300/50 scrollbar-thin">
        ...
        <div className="max-w-[800px] mx-auto bg-white p-12 md:p-20 shadow-2xl min-h-[1050px] relative text-slate-900 ring-1 ring-slate-950/5">
          ...
          <div className="text-center mb-10 pb-8">
            <h1 className="text-4xl font-bold uppercase tracking-tighter mb-1 font-serif">
              {originalData.personal.name}
            </h1>
            <div className="relative group inline-block w-full">
              <p
                className={`text-lg font-bold text-slate-700 font-serif ${originalData.personal.title !== revisedData.personal.title ? 'bg-amber-100 border-b-2 border-amber-400 px-2 rounded-sm inline-block' : ''}`}
              >
              ...
              {originalData.personal.title !== revisedData.personal.title && (
                <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-3 hidden group-hover:block w-72 p-4 bg-slate-900 text-white text-xs rounded-xl shadow-2xl z-50 normal-case font-sans italic leading-relaxed border border-amber-400/30">
                  ...
                  <p>
                    Strategically re-titled from{' '}
                    <span className="text-slate-400">"{originalData.personal.title}"</span> to
                    maximize ATS matching for targeted engineering roles.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-x-3 gap-y-1 mt-5 text-[11px] font-sans text-slate-500 font-medium">
              <span className="flex items-center gap-1.5 underline underline-offset-2 decoration-slate-300 truncate">
                <Mail className="w-3 h-3" /> {revisedData.personal.email}
              </span>
              <span className="text-slate-300">&bull;</span>
              <span className="flex items-center gap-1.5">
                <Phone className="w-3 h-3" /> {revisedData.personal.phone}
              </span>
              <span className="text-slate-300">&bull;</span>
              <span className="flex items-center gap-1.5">
                <MapPin className="w-3 h-3" /> {revisedData.personal.location}
              </span>
            </div>
          </div>
          ...
          <div className="mb-12 group relative">
            <h2 className="text-[12px] font-bold uppercase tracking-[0.2em] border-b-2 border-slate-900 pb-1 mb-4 text-slate-900">
              ...
              {originalData.personal.bio !== revisedData.personal.bio && (
                <div className="absolute -right-4 top-0 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <div className="absolute left-full ml-4 top-0 hidden group-hover:block w-72 p-4 bg-slate-900 text-white text-xs rounded-2xl shadow-2xl z-50 font-sans normal-case leading-relaxed border border-amber-500/20">
                    ...
                    Narrative expanded to integrate high-density keywords (ATS optimization) while
                    quantifying your top achievements.
                  </div>
                </div>
              )}
            </div>
          ...
          {/* Experience Section */}
          <div className="mb-12">
            <h2 className="text-[12px] font-bold uppercase tracking-[0.2em] border-b-2 border-slate-900 pb-1 mb-6 text-slate-900">
                ...
                return (
                  <div key={exp.id} className="relative">
                    ...
                    <div className="flex justify-between items-baseline mb-3">
                      ...
                      <p className="text-[11px] text-slate-500 font-sans font-medium uppercase tracking-wider">
                        {exp.location}
                      </p>
                    </div>
                              ...
                              <div className="absolute -left-12 top-0 bottom-0 flex items-center opacity-30 group-hover:opacity-100 transition-opacity">
                                ...
                                <div className="absolute bottom-full left-0 mb-4 hidden group-hover:block w-80 p-5 bg-slate-900 text-white text-xs rounded-2xl shadow-2xl z-50 font-sans normal-case leading-normal border border-indigo-500/30">
                                  ...
                                  <div className="space-y-3">
                                    <p className="text-slate-400 italic bg-slate-950 p-2 rounded-lg border border-slate-800">
                                      Original: "{origExp?.description?.[bIdx] || ''}"
                                    </p>
                                    <p className="text-emerald-400 font-bold border-t border-slate-800 pt-3">
                                      Impact analysis complete. Bullet re-written to emphasize
                                      "Result-driven" metrics and "Scale" to catch recruiter
                                      attention.
                                    </p>
                                  </div>
                                </div>
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-3 text-[13px]">
              {revisedData.skills.map((skill, sIdx) => {
                ...
                return (
                  <span
                    key={sIdx}
                    className={`group relative cursor-help ${isNew ? 'bg-indigo-100 text-indigo-800 font-bold px-2 py-0.5 rounded-md border border-indigo-200' : 'text-slate-800'}`}
                  >
                    {skill.name}
                    {isNew && (
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-wrap gap-2.5">
            {appliedFixes.slice(0, 3).map((f, i) => (
              <div
                key={i}
                className="flex items-center gap-1.5 bg-indigo-50 text-indigo-700 border border-indigo-100 px-4 py-1 rounded-lg font-bold text-[10px] shadow-sm"
              >
            ...
            {appliedFixes.length > 3 && (
              <span className="text-[10px] text-slate-400 font-bold bg-slate-50 px-3 py-1 rounded-lg border border-slate-100">
                +{appliedFixes.length - 3} MORE FIXES
              </span>
            )}
          </div>
        </div>
