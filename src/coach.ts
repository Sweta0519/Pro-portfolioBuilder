...
function hasMetrics(bullet: string): boolean {
  ...
  return (
    /%|\$\d+|\d+(?:\s*|-)(?:x|KB|MB|GB|ms|s|h|hours|days|weeks|months|years|x|percent|times|million|k|thousand|projects|people|engineers|developers|users|clients|merchant)/i.test(
      bullet
    ) || /\b\d{2,}\b/.test(bullet)
  );
}
...
export function analyzeResume(data: ResumeData): CoachScoreAnalysis {
  ...
  if (data.personal.title && data.personal.title !== 'Software Professional / Domain Expert') {
    ...
    recommendations.push({
      ...
      description:
        'Avoid generic titles. Use targeted names like "Senior Product Engineer" or "UX/UI Designer" to stand out.',
      category: 'personal',
      completed: false,
      scoreImpact: 5,
    });
  ...
  if (data.personal.bio && data.personal.bio.length > 50) {
    ...
    if (data.personal.bio.length > 120) {
      ...
      recommendations.push({
        ...
        description:
          'Elaborate on your core expertise, industry experience, and what problems you love solving (aim for 120+ characters).',
        category: 'personal',
        completed: false,
        scoreImpact: 5,
      });
    ...
    recommendations.push({
      ...
      description:
        'Your bio is the first thing recruiters read. Highlight your key achievements and developer identity.',
      category: 'personal',
      completed: false,
      scoreImpact: 10,
    });
  ...
  if (socialCount >= 3) {
    ...
    recommendations.push({
      ...
      description:
        'Connecting professional profiles increases interview rates by 40%. Provide at least 3 active profiles.',
      category: 'personal',
      completed: false,
      scoreImpact: 5,
    });
  ...
  if (data.experience && data.experience.length > 0) {
    ...
    if (totalBullets > 0) {
      ...
      recommendations.push({
        ...
        description:
          'List key achievements and projects for each professional role, rather than just listing company names.',
        category: 'experience',
        completed: false,
        scoreImpact: 15,
      });
    ...
    recommendations.push({
      ...
      description:
        'Add your history of full-time, contract, or freelance positions to prove experience.',
      category: 'experience',
      completed: false,
      scoreImpact: 30,
    });
  ...
  if (data.skills && data.skills.length >= 8) {
    ...
    if (data.skills.length >= 15) {
      ...
      recommendations.push({
        ...
        description:
          'Add a broader set of skills (15+) to cover frontend, backend, tools, and methodologies.',
        category: 'skills',
        completed: false,
        scoreImpact: 5,
      });
    ...
    if (categories.size >= 4) {
      ...
      recommendations.push({
        ...
        description:
          'Organize your skills into different domains (Frontend, Backend, Cloud/DevOps, Design) to show cross-functional abilities.',
        category: 'skills',
        completed: false,
        scoreImpact: 5,
      });
    ...
    recommendations.push({
      ...
      description:
        'List the programming languages, libraries, frameworks, and tools you are proficient in.',
      category: 'skills',
      completed: false,
      scoreImpact: 15,
    });
  ...
  if (data.certificates && data.certificates.length > 0) {
    ...
    recommendations.push({
      ...
      description:
        'Add professional certifications, bootcamps, or online courses to establish credibility.',
      category: 'skills',
      completed: false,
      scoreImpact: 5,
    });
  ...
  if (data.projects && data.projects.length > 0) {
    ...
    if (data.projects.length >= 3) {
      ...
      recommendations.push({
        ...
        description:
          'Aim for at least 3 featured projects in your portfolio to demonstrate diverse engineering challenges.',
        category: 'projects',
        completed: false,
        scoreImpact: 5,
      });
    ...
    if (hasLinks) {
      ...
      recommendations.push({
        ...
        description:
          'Add live demo URL or source code links (GitHub) to your projects so visitors can inspect your work.',
        category: 'projects',
        completed: false,
        scoreImpact: 5,
      });
    ...
    recommendations.push({
      ...
      description:
        'Your code speaks louder than a dry list of words. Add real, interactive projects to prove your capabilities.',
      category: 'projects',
      completed: false,
      scoreImpact: 20,
    });
  ...
  recommendations.forEach((rec) => {
    ...
    if (rec.id === 'name' && data.personal.name && data.personal.name !== 'Professional Candidate')
      rec.completed = true;
    if (
      rec.id === 'title' &&
      data.personal.title &&
      data.personal.title !== 'Software Professional / Domain Expert'
    )
      rec.completed = true;
    if (rec.id === 'bio-missing' && data.personal.bio && data.personal.bio.length > 50)
      rec.completed = true;
    if (rec.id === 'bio-length' && data.personal.bio && data.personal.bio.length > 120)
      rec.completed = true;
    if (
      rec.id === 'social-links' &&
      Object.values(data.personal.socials).filter(Boolean).length >= 3
    )
      rec.completed = true;
    if (rec.id === 'exp-missing' && data.experience && data.experience.length > 0)
      rec.completed = true;
    if (rec.id === 'exp-count' && data.experience && data.experience.length >= 2)
      rec.completed = true;
    ...
    if (
      rec.id === 'skills-categories' &&
      data.skills &&
      new Set(data.skills.map((s) => s.category)).size >= 4
    )
      rec.completed = true;
    if (rec.id === 'certs-missing' && data.certificates && data.certificates.length > 0)
      rec.completed = true;
    if (rec.id === 'projects-missing' && data.projects && data.projects.length > 0)
      rec.completed = true;
    if (rec.id === 'projects-count' && data.projects && data.projects.length >= 3)
      rec.completed = true;
    if (
      rec.id === 'projects-links' &&
      data.projects &&
      data.projects.some((p) => p.link || p.github)
    )
      rec.completed = true;
  });
