...
export function analyzeATSCompliance(data: ResumeData, jobDescription: string): ATSAnalysisResult {
  ...
  // LinkedIn Check
  const hasLinkedIn =
    data.personal.socials.linkedin && data.personal.socials.linkedin.includes('linkedin.com');
  if (hasLinkedIn) {
    complianceChecks.push({
      id: 'linkedin',
    ...
    complianceChecks.push({
      ...
      description:
        'Jobscan Recommendation: Adding a LinkedIn URL raises recruiter call rates by 28%.',
      status: 'warning',
      scoreImpact: -10,
    });
  }
  ...
  complianceChecks.push({
    ...
    description:
      'Layout avoids unreadable canvas charts and complicated column grids that trip up older parsers.',
    status: 'pass',
    scoreImpact: 0,
  });
  ...
  if (wordCount < 200) {
    ...
    wordCountTip =
      'Critical: Your resume is too brief (< 200 words). Add detailed work history bullet points.';
    complianceScore -= 10;
  } else if (wordCount < 400) {
    wordCountStatus = 'warning';
    wordCountTip =
      'Warning: A bit short. Expand on technical tasks and project results to beat heavy competition.';
  } else if (wordCount > 1000) {
    wordCountStatus = 'warning';
    wordCountTip =
      'Warning: Too long (> 1000 words). Focus on high-impact bullets to avoid recruiter fatigue.';
  }
  ...
  if (avgSentenceLength > 25) {
    readStatus = 'warning';
    readTip =
      'Sentences are too long (> 25 words average). Shorten complex run-on lines for higher readability.';
  } else if (avgSentenceLength < 8) {
    readStatus = 'warning';
    readTip =
      'Sentences are very short, which might sound choppy. Add professional transition words.';
  }
  ...
  const targetHard = jobDescription
    ? HARD_SKILLS.filter((k) => jdLower.includes(k))
    : HARD_SKILLS.slice(0, 12);
  const targetSoft = jobDescription
    ? SOFT_SKILLS.filter((k) => jdLower.includes(k))
    : SOFT_SKILLS.slice(0, 6);
  ...
  const matchScore =
    keywordCountTotal > 0 ? Math.round((keywordMatches / keywordCountTotal) * 100) : 70;
...
export function analyzeCoverLetter(
  ...
  if (wordCount < 150) {
    ...
    insights.push(
      'Cover Letter is exceptionally long (> 450 words). Keep it concise to maintain recruiter interest.'
    );
  } else {
  ...
  const targetKeywords = jdLower
    ? HARD_SKILLS.concat(SOFT_SKILLS).filter((k) => jdLower.includes(k))
    : HARD_SKILLS.slice(0, 8);
  ...
  const keywordMatches = matchedKeywords.length;
  const score =
    targetKeywords.length > 0 ? Math.round((keywordMatches / targetKeywords.length) * 100) : 60;
...
export function autoOptimizeResume(
  data: ResumeData,
  jobDescription: string
): { revisedData: ResumeData; fixes: string[] } {
  const fixes: string[] = [];
  const jdLower = jobDescription.toLowerCase();
    ...
    if (foundTitles.length > 0) {
      const targetTitle =
        foundTitles
          ...
          .join(' ') + (foundTitles.includes('manager') ? '' : ' Specialist');
  ...
  if (missingKeywords.length > 0) {
    ...
    if (added.length > 0) {
      fixes.push(
        `Strategically integrated missing technical and soft keywords: ${added.join(', ')}.`
      );
    }
  }
  ...
  if (verbUpgrades > 0) {
    fixes.push(
      `Upgraded ${verbUpgrades} action verbs to more impactful professional alternatives.`
    );
  }
