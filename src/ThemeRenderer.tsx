...
const ThemeSection: React.FC<{
  ...
  textHeading?: string;
  children: React.ReactNode;
}> = ({ title, id, textHeading = 'text-white', children }) => (
  <section id={id} className="scroll-mt-20 py-10 scroll-slide">
    <div className="flex items-center gap-4 mb-8">
      <h2
        className={`text-2xl md:text-3xl font-black ${textHeading} whitespace-nowrap tracking-tight`}
      >
        {title}
      </h2>
...
export const ThemeRenderer: React.FC<ThemeRendererProps> = ({
    ...
    return classes
      ...
      .filter((cls) => {
        ...
        if (previewDevice === 'tablet') {
          return !cls.startsWith('lg:') && !cls.startsWith('xl:') && !cls.startsWith('2xl:');
        }
        return true;
      })
      .join(' ');
    ...
    return (
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div
            className={`absolute top-[-10%] left-[-10%] w-[60%] h-[60%] rounded-full opacity-20 blur-[120px] ${primaryBg} animate-pulse`}
          ></div>
          <div
            className="absolute bottom-[-10%] right-[-10%] w-[60%] h-[60%] rounded-full opacity-20 blur-[120px] bg-indigo-500 animate-pulse"
        ...
        <div className={rc('relative z-10 flex flex-col md:flex-row min-h-screen')}>
          ...
          <aside
            className={rc(
              `w-full md:w-80 md:min-h-screen p-6 md:p-8 md:sticky md:top-0 flex flex-col backdrop-blur-[20px] ${sidebarBg}`
            )}
          >
            <div className="flex flex-col items-center md:items-start text-center md:text-left">
              <div
                className={`w-24 h-24 md:w-32 md:h-32 rounded-3xl mb-4 md:mb-6 overflow-hidden border border-slate-200 dark:border-white/20 shadow-2xl rotate-3 hover:rotate-0 transition-transform duration-500 ${primaryBg}/20 flex items-center justify-center text-3xl md:text-4xl font-black ${textHeading} relative group`}
              >
              ...
              <h1
                className={rc(
                  `text-2xl md:text-3xl font-black ${textHeading} tracking-tight mb-2 leading-tight break-words w-full`
                )}
              >
                {data.personal.name}
              </h1>
              <p
                className={`text-xs md:text-sm font-bold uppercase tracking-widest ${primaryClass}`}
              >
                {data.personal.title}
              </p>
              ...
              <div className="mt-6 md:mt-8 space-y-3 w-full text-left">
                ...
                {data.personal.location && (
                  <div
                    className={`flex items-center gap-3 ${textBody} text-xs ${cardBg} p-2.5 md:p-3 rounded-xl border`}
                  >
            ...
            <nav
              className={rc(
                'mt-6 md:mt-12 flex flex-row md:flex-col gap-2 md:space-y-1 overflow-x-auto pb-2 md:pb-0 scrollbar-none justify-start w-full'
              )}
            >
              {[
                'About',
                ...
                'Projects',
                'Skills',
                ...(data.certificates && data.certificates.length > 0 ? ['Awards'] : []),
                'Contact',
              ].map((item) => (
                <a
                  key={item}
          ...
          <main className={rc('flex-1 p-5 md:p-16 max-w-5xl overflow-hidden')}>
            <section id="about" className="mb-16 md:mb-24 animate-fadeIn scroll-slide scroll-mt-20">
              <h2
                className={rc(
                  `text-3xl md:text-6xl font-black ${textHeading} mb-6 md:mb-8 leading-tight tracking-tighter`
                )}
              >
                Crafting{' '}
                <span
                  className={`${primaryClass} underline decoration-slate-200 dark:decoration-white/10 decoration-8 underline-offset-4`}
                >
                  impactful
                </span>{' '}
                digital products.
              ...
              <div
                className={rc(
                  `backdrop-blur-md ${cardBg} border rounded-2xl md:rounded-3xl p-6 md:p-10 relative overflow-hidden group`
                )}
              >
                <div
                  className={`absolute -right-10 -bottom-10 w-40 h-40 ${primaryBg} opacity-5 blur-3xl rounded-full`}
                ></div>
                ...
                <div
                  className={rc(
                    'grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6 md:mt-8 pt-6 md:pt-8 border-t border-slate-200 dark:border-white/10'
                  )}
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-10 h-10 rounded-xl ${pillBg} border flex items-center justify-center`}
                    >
                    ...
                    <div>
                      ...
                      <div className={`text-sm font-bold ${textHeading}`}>
                        {data.personal.title}
                      </div>
                    </div>
                  </div>
                ...
                {data.experience.map((exp, idx) => (
                  ...
                  >
                    ...
                    <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                      <h3
                        className={rc(
                          `text-xl md:text-2xl font-bold ${textHeading} tracking-tight`
                        )}
                      >
                        {exp.position}
                      </h3>
                      <span className={`text-xs font-bold ${pillBg} px-3 py-1 rounded-full border`}>
                        {exp.period}
                      </span>
                    </div>
                    <p className={`text-sm md:text-base font-bold ${primaryClass} mb-3 md:mb-4`}>
                      {exp.company}
                    </p>
                    <ul className="space-y-3">
                      {exp.description.map((bullet, bIdx) => (
                        <li
                          key={bIdx}
                          className={`${textMuted} text-xs md:text-sm leading-relaxed flex gap-3`}
                        >
                          <span className={`${primaryClass} mt-1.5 font-black flex-shrink-0`}>
                            →
                          </span>
                          <span>{bullet}</span>
                        </li>
            ...
            <ThemeSection title="Projects" id="projects">
              <div className={rc('grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8')}>
                {data.projects.map((proj, idx) => (
                  ...
                  >
                    <div
                      className={`absolute -right-4 -top-4 w-24 h-24 ${primaryBg} opacity-5 blur-2xl group-hover:opacity-20 transition-opacity`}
                    ></div>
                    <div
                      className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl mb-4 md:mb-6 ${primaryBg}/20 flex items-center justify-center ${primaryClass} border border-slate-200 dark:border-white/5`}
                    >
                      <Code className="w-6 h-6 md:w-7 md:h-7" />
                    </div>
                    <h3
                      className={rc(
                        `text-xl md:text-2xl font-bold ${textHeading} mb-2 md:mb-3 tracking-tight`
                      )}
                    >
                      {proj.title}
                    </h3>
                    <p
                      className={rc(
                        `text-xs md:text-sm ${textBody} mb-6 md:mb-8 leading-relaxed line-clamp-4 flex-grow`
                      )}
                    >
                      {proj.description}
                    </p>
                  ...
                  {data.certificates.map((cert) => (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <h3
                          className={rc(
                            `text-base md:text-lg font-bold ${textHeading} tracking-tight`
                          )}
                        >
                          {cert.name}
                        </h3>
            ...
            <footer className="mt-20 py-8 border-t border-slate-200 dark:border-white/5 text-center">
              <p className="text-[10px] md:text-xs font-bold text-slate-500 tracking-widest uppercase">
                © {new Date().getFullYear()} {data.personal.name} • Built with ProPortfolio Builder
                • Developed by Swetaprangya Sahoo
              </p>
            </footer>
          </main>
        </div>
  ...
  if (settings.id === 'gradient') {
    return (
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div
            className={`absolute top-[-20%] left-[-10%] w-[70%] h-[70%] rounded-full opacity-30 blur-[150px] ${primaryBg} animate-pulse`}
          ></div>
          <div
            className="absolute bottom-[-20%] right-[-10%] w-[70%] h-[70%] rounded-full opacity-30 blur-[150px] bg-indigo-600 animate-pulse"
            style={{ animationDelay: '2s' }}
          ></div>
        </div>
        ...
        >
          <header
            className={rc('text-center mb-16 md:mb-32 animate-fadeIn flex flex-col items-center')}
          >
            <div
              className={rc(
                `inline-block px-4 py-2 rounded-full ${pillBg} border text-[10px] font-black uppercase tracking-[0.3em] ${primaryClass} mb-6 md:mb-12`
              )}
            >
            ...
            <h1
              className={rc(
                `text-4xl md:text-8xl lg:text-9xl font-black ${textHeading} tracking-tighter mb-6 md:mb-10 leading-[0.85] break-words w-full px-4`
              )}
            >
              {data.personal.name.split(' ')[0]} <br />
              <span
                className={`bg-gradient-to-r ${primaryClass.replace('text-', 'from-')} to-indigo-400 bg-clip-text text-transparent break-words`}
              >
            ...
            <p
              className={rc(
                `text-lg md:text-2xl ${textBody} max-w-2xl mx-auto leading-relaxed font-bold tracking-tight px-2`
              )}
            >
            ...
            <div
              className={rc(
                'mt-8 md:mt-16 flex flex-wrap justify-center gap-4 md:gap-6 w-full px-4'
              )}
            >
              <a
                href="#contact"
            ...
            <ThemeSection title="About Me" id="about">
              ...
              >
                <div className="flex-1 space-y-4">
                  <h3
                    className={rc(`text-2xl md:text-3xl font-black ${textHeading} leading-tight`)}
                  >
                    Behind the{' '}
                    <span
                      className={`bg-gradient-to-r ${primaryClass.replace('text-', 'from-')} to-indigo-400 bg-clip-text text-transparent`}
                    >
                      craft
                    </span>
                    .
                  ...
                  <DiffHighlight current={data.personal.bio} original={originalData?.personal.bio}>
                    <p
                      className={rc(`text-base md:text-xl ${textBody} leading-relaxed font-medium`)}
                    >
                      {data.personal.bio}
                    </p>
                  </DiffHighlight>
                ...
                {data.personal.avatar && (
                  <div className="w-36 h-36 md:w-48 md:h-48 rounded-[2rem] overflow-hidden border border-slate-200 dark:border-white/10 shadow-2xl relative group shrink-0">
                    ...
                    {data.personal.avatar.length > 2 ? (
                      ...
                      <div
                        className={`w-full h-full flex items-center justify-center ${textHeading} text-5xl md:text-7xl font-black bg-slate-100 dark:bg-white/5`}
                      >
                        {data.personal.avatar}
                      </div>
                    )}
              ...
              <div className={rc('space-y-12 md:space-y-24')}>
                {data.experience.map((exp, idx) => (
                  <div
                    key={idx}
                    className={rc(
                      'group grid grid-cols-1 md:grid-cols-[1fr_2.5fr] gap-4 md:gap-12'
                    )}
                  >
                    ...
                    <div>
                      ...
                      <p
                        className={rc(`text-sm md:text-lg font-bold ${primaryClass} mb-4 md:mb-8`)}
                      >
                        {exp.company}
                      </p>
                      <div className={rc('space-y-3 md:space-y-6')}>
                        {exp.description.map((bullet, bIdx) => (
                          <p
                            key={bIdx}
                            className={rc(
                              `${textBody} text-sm md:text-lg leading-relaxed font-medium`
                            )}
                          >
                            {bullet}
                          </p>
            ...
            <ThemeSection title="Selected Projects" id="projects">
              <div className={rc('space-y-10 md:space-y-20')}>
                {data.projects.map((proj, idx) => (
                  ...
                  >
                    <div
                      className={`absolute -right-20 -top-20 w-80 h-80 ${primaryBg} opacity-5 blur-[100px] group-hover:opacity-10 transition-opacity`}
                    ></div>
                    <div
                      className={`text-[10px] md:text-xs font-black uppercase tracking-[0.3em] ${primaryClass} mb-4 md:mb-6`}
                    >
                      {proj.category || 'Project'}
                    </div>
                    <h3
                      className={rc(
                        `text-2xl md:text-5xl font-black ${textHeading} mb-4 md:mb-8 group-hover:translate-x-3 transition-transform leading-none`
                      )}
                    >
                      {proj.title}
                    </h3>
                    <p
                      className={rc(
                        `text-sm md:text-xl ${textBody} mb-6 md:mb-10 max-w-xl leading-relaxed font-medium`
                      )}
                    >
                      {proj.description}
                    </p>
                ...
                {data.skills.map((skill) => (
                  <div key={skill.name} className="space-y-3 md:space-y-4">
                    <div className="flex justify-between items-end">
                      <span
                        className={rc(
                          `text-base md:text-xl font-black ${textHeading} tracking-tight`
                        )}
                      >
                        {skill.name}
                      </span>
                      <span className={`text-xs md:text-sm font-black ${primaryClass}`}>
                        {skill.level}%
                      </span>
                    </div>
              ...
              <ThemeSection title="Awards & Recognitions" id="awards">
                <div className={rc('grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8')}>
                  {data.certificates.map((cert) => (
                    ...
                    >
                      <div
                        className={`absolute -right-10 -top-10 w-24 h-24 ${primaryBg} opacity-5 blur-2xl group-hover:opacity-10 transition-opacity`}
                      ></div>
                      ...
                      <div className="text-left">
                        <div className="text-[10px] md:text-xs font-black uppercase tracking-[0.2em] text-slate-500 mb-2">
                          {cert.issuer} {cert.date ? `• ${cert.date}` : ''}
                        </div>
                        <h3
                          className={rc(
                            `text-xl md:text-2xl font-black ${textHeading} leading-none mb-2`
                          )}
                        >
                          {cert.name}
                        </h3>
        ...
        <footer className="w-full py-12 md:py-20 px-6 border-t border-white/5 text-center">
          <p className="text-[10px] md:text-sm font-bold text-slate-500 tracking-widest uppercase">
            © {new Date().getFullYear()} {data.personal.name} • Built with ProPortfolio Builder •
            Developed by Swetaprangya Sahoo
          </p>
        </footer>
  ...
  const renderHero = () => {
    if (settings.id === 'classic') {
      return (
        <div
          id="about"
          className="py-12 px-8 border-b-2 border-slate-900 bg-white text-slate-900 font-serif"
        >
            ...
            <div className="flex flex-wrap justify-center gap-x-4 gap-y-1 text-sm font-sans text-slate-600">
              {data.personal.email && (
                <span className="flex items-center gap-1">
                  <Mail className="w-3 h-3" /> {data.personal.email}
                </span>
              )}
              {data.personal.phone && (
                <span className="flex items-center gap-1">
              ...
              {data.personal.location && (
                <span className="flex items-center gap-1">
                  <MapPin className="w-3 h-3" /> {data.personal.location}
                </span>
              )}
            </div>
            <div className="pt-4 border-t border-slate-200">
              <p className="text-lg font-medium italic text-slate-700">{data.personal.title}</p>
              <div className="text-[11px] font-bold uppercase tracking-widest text-slate-500 mt-4 mb-2">
                Professional Summary
              </div>
              <DiffHighlight
                current={data.personal.bio}
                original={originalData?.personal.bio}
                inline
              >
    ...
    return (
      <section
        id="about"
        className={rc('relative py-12 md:py-32 px-6 md:px-12 overflow-hidden scroll-mt-20')}
      >
        <div
          className={rc(
            'max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-5 gap-10 lg:gap-16 items-center'
          )}
        >
          <div className={rc('lg:col-span-3 space-y-6 md:space-y-8')}>
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <span className={`text-xs font-bold uppercase tracking-[0.2em] ${colors.text}`}>
                  {data.personal.title}
                </span>
              </div>
              <h1
                className={rc(
                  `text-4xl md:text-7xl font-black tracking-tighter leading-[0.9] ${textHeading}`
                )}
              >
                {data.personal.name}
              </h1>
          ...
          <div className={rc('lg:col-span-2 relative')}>
            ...
            <div
              className={`absolute -bottom-8 -right-8 w-40 h-40 ${colors.bg} opacity-20 blur-3xl rounded-full`}
            ></div>
          </div>
        </div>
      </section>
  ...
  return (
    ...
    >
      ...
      {!isClassic && (
        <header className={`sticky top-0 z-50 backdrop-blur-lg ${headerBg} px-6 py-4`}>
          <div className="max-w-5xl mx-auto flex justify-between items-center">
            <div className="flex items-center gap-3">
              <div
                className={`w-8 h-8 rounded-lg ${colors.bg} flex items-center justify-center text-white font-black text-xs`}
              >
                {data.personal.name.substring(0, 1)}
              </div>
              <span className={`text-sm font-black tracking-tight ${textHeading} uppercase`}>
                {data.personal.name}
              </span>
            </div>
                ...
                <div className="space-y-8">
                  ...
                  <div className="flex justify-between items-center pb-4 border-b border-slate-100 dark:border-white/5">
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-6 h-6 rounded-lg ${colors.bg} flex items-center justify-center text-white font-black text-[10px]`}
                      >
                        {data.personal.name.substring(0, 1)}
                      </div>
                  ...
                  <nav className="flex flex-col space-y-2">
                    {[
                      ...
                      >
                        <span>{item}</span>
                        <span
                          className={`opacity-0 group-hover:opacity-100 transition-opacity text-xs ${colors.text}`}
                        >
                          →
                        </span>
                      </a>
      ...
      <main
        className={rc(
          `flex-grow max-w-5xl mx-auto px-6 py-12 md:py-24 space-y-16 md:space-y-32 w-full ${isClassic ? 'bg-white text-slate-900 font-serif max-w-4xl py-12' : ''}`
        )}
      >
                ...
                <div className="pt-6 border-t border-slate-900 grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                  ...
                  <div className="flex gap-2">
                    ...
                    <span className="text-slate-400">
                      {data.personal.location || 'REMOTE_ACCESS'}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <section id="experience" className="scroll-mt-24 scroll-slide">
          <div className="flex items-center gap-4 mb-12">
            <h2
              className={`text-3xl font-black tracking-tight ${isClassic ? 'text-xl uppercase border-b-2 border-slate-900 pb-1 w-full' : textHeading}`}
            >
          ...
          <div className="space-y-16">
            {data.experience.map((exp, idx) => (
              ...
              >
                {!isClassic && (
                  <div
                    className={`absolute left-[-4px] top-0 w-2 h-2 rounded-full ${colors.bg} opacity-0 group-hover:opacity-100 transition-opacity`}
                  ></div>
                )}
                <div className="flex flex-wrap justify-between items-baseline mb-2 gap-2">
                  <h3
                    className={`text-2xl font-bold ${isClassic ? 'text-slate-900' : textHeading}`}
                  >
                    {exp.position}
                  </h3>
                ...
                <div
                  className={`text-lg font-bold ${isClassic ? 'text-slate-800 mb-4' : `${colors.text} mb-6`}`}
                >
                  {exp.company} • <span className="font-medium opacity-60">{exp.location}</span>
                </div>
                <ul className="space-y-3">
                  {exp.description.map((bullet, bIdx) => (
                    <li
                      key={bIdx}
                      className={`text-base leading-relaxed ${isClassic ? 'text-slate-700 list-disc ml-5' : `${textBody} flex gap-3`}`}
                    >
          ...
          <div className={rc('grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8')}>
            {data.projects.map((proj, idx) => (
              ...
              >
                ...
                <p
                  className={`text-base mb-6 leading-relaxed ${isClassic ? 'text-slate-700' : textBody}`}
                >
                  {proj.description}
                </p>
                <div className="flex flex-wrap gap-2">
                  {proj.techStack.map((tech) => (
                    <span
                      key={tech}
                      className={`text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-full ${isClassic ? 'bg-slate-100 text-slate-600' : `${pillBg} border`}`}
                    >
                      {tech}
                    </span>
                  ))}
        ...
        <div className={rc('grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-20')}>
          <section id="education">
            <h2
              className={`text-2xl font-black mb-10 ${isClassic ? 'text-lg uppercase border-b-2 border-slate-900 pb-1' : textHeading}`}
            >
              Education
            </h2>
            <div className="space-y-8">
              {data.education.map((edu, idx) => (
                <div key={idx}>
                  <h3
                    className={`text-lg font-bold mb-1 ${isClassic ? 'text-slate-900' : textHeading}`}
                  >
                    {edu.degree}
                  </h3>
                  <p
                    className={`text-base font-bold ${isClassic ? 'text-slate-800' : colors.text} mb-1`}
                  >
                    {edu.institution}
                  </p>
                  <p className="text-sm text-slate-500">
                    {edu.fieldOfStudy} • {edu.period}
                  </p>
                </div>
              ))}
            </div>
              ...
              {data.certificates.map((cert) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <p
                      className={`text-sm ${isClassic ? 'text-slate-600' : 'text-slate-400'} mt-1`}
                    >
                      {cert.issuer} {cert.date ? `• ${cert.date}` : ''}
                    </p>
                  </div>
        ...
        {!isClassic && (
          <section id="contact">
            <div
              className={rc(
                'p-6 md:p-12 rounded-2xl md:rounded-[3rem] bg-slate-900 text-white relative overflow-hidden'
              )}
            >
              <div
                className={rc(
              ...
              >
                ...
                <p className="text-slate-400 max-w-xl text-sm md:text-lg font-medium">
                ...
                <button
                  className={rc(
                    `mt-6 md:mt-8 px-8 py-4 md:px-12 md:py-5 rounded-full ${colors.bg} text-white font-black uppercase tracking-[0.2em] text-[10px] md:text-xs hover:scale-105 transition-transform w-full sm:w-auto`
                  )}
                >
