...
function cleanBinaryContent(text: string): string {
  if (!text.includes('%PDF') && !text.includes('PK\x03\x04') && !text.includes('[Content_Types].xml')) {
    return text;
  }
      ...
      recovered = matches
        ...
        .filter(str => {
          ...
          return (
            ...
            !str.includes('/') && 
            !/^[0-9.\s()\[\]{}]+$/.test(str) &&
            !['font', 'helvetica', 'arial', 'times', 'roman', 'encoding', 'identity', 'flatedecode'].some(g => s.includes(g))
          );
        })
        .join('\n');
    }
...
const GARBAGE_TERMS = [
  'en-gb', 'en-us', 'curriculum vitae', 'resume', 'cv', 'page 1', 'page 2', 
...
export function parseRawResumeText(rawText: string): Partial<ResumeData> {
  ...
  for (let i = 0; i < Math.min(4, lines.length); i++) {
    ...
    const nameBlacklist = ['engineer', 'developer', 'analyst', 'manager', 'architect', 'designer', 'specialist', 'lead', 'support', 'technology', 'solutions', 'services', 'gmbh', 'inc', 'corp', 'co', 'limited', 'ltd', 'summary', 'experience', 'education', 'skills', 'about', 'profile', 'work', 'project', 'dxc'];
  ...
  if (potentialName) {
    ...
    const fallbackIdx = lines.findIndex(l => {
      ...
      const isBullet = /^[•\-\*▪◦⁃‣✓✔★]/.test(clean) || /^\d+\.\s/.test(clean) || /^o\s/.test(clean);
      return clean.length >= 3 && clean.length <= 80 && !isBullet && !GARBAGE_TERMS.some(term => lower === term || lower.includes(term));
    });
    if (fallbackIdx !== -1) {
      let fallbackName = lines[fallbackIdx].split(/[|•,]/)[0].trim();
      const titleKeywords = /engineer|developer|analyst|manager|architect|designer|specialist|lead|support|officer|consultant|expert|head/i;
  ...
  for (let i = nameLineIdx + 1; i < Math.min(nameLineIdx + 6, lines.length); i++) {
    ...
    if (
      ...
      line.length < 100 &&
      !line.includes('@') &&
      (lower.includes('engineer') || lower.includes('developer') || lower.includes('architect') || 
       lower.includes('designer') || lower.includes('analyst') || lower.includes('manager') || 
       lower.includes('specialist') || lower.includes('consultant') || lower.includes('lead') || 
       lower.includes('stack') || lower.includes('qa') || lower.includes('tester'))
    ) {
  ...
  const firstHeaderIdx = lines.findIndex((l, idx) => idx > nameLineIdx && /^(experience|work experience|professional experience|education|skills|summary|profile|about me|additional)/i.test(l));
  ...
  for (let i = nameLineIdx + 1; i < Math.min(bioLimit, lines.length); i++) {
    ...
    if (line === parsed.personal!.title || line.replace(/ /g, '_') === parsed.personal!.title.replace(/ /g, '_')) continue;
  ...
  parsed.personal!.avatar = parsed.personal!.name.split(' ').map(n => n[0]).join('').toUpperCase().substring(0, 2);
  ...
  let currentSection: 'experience' | 'education' | 'skills' | 'certificates' | 'summary' | 'none' = 'none';
  let currentExp: Partial<WorkExperience> | null = null;
  let currentEdu: Partial<Education> | null = null;

  const flushActiveTimelines = () => {
    if (currentSection === 'experience' && currentExp && currentExp.company && currentExp.position) {
      if (!experiences.some(e => e.company === currentExp!.company && e.position === currentExp!.position)) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^(awards|certifications|certificates):\s*/i.test(line)) {
      ...
      parts.forEach(part => {
        if (part.length > 5) {
          ...
          let textWithoutDate = part.replace(/\s*\(\s*(19|20)\d{2}\s*\)/, '').replace(/\s*[,]?\s*\b(19|20)\d{2}\b/, '').trim();
    ...
    if (/^(technical skills|tools & technologies|tools and technologies|key skills|skills|tools|technologies|languages):\s*/i.test(line)) {
      flushActiveTimelines();
      const prefixMatch = line.match(/^(technical skills|tools & technologies|tools and technologies|key skills|skills|tools|technologies|languages):\s*/i);
    ...
    if (isShortLine && /^(experience|work experience|professional experience|employment history|employment|work history|history|career|experience:)/i.test(line)) {
      flushActiveTimelines();
      currentSection = 'experience';
      currentExp = null;
      continue;
    } else if (isShortLine && /^(education|academic background|university education|academics|academic|education:)/i.test(line)) {
      flushActiveTimelines();
      currentSection = 'education';
      currentEdu = null;
      continue;
    } else if (isShortLine && (/^(skills|technical skills|key skills|core competencies|expertise|tech stack|technologies|technologies:)/i.test(line) || lower === 'skills' || lower === 'technical skills' || lower === 'expertise' || lower === 'technologies')) {
      flushActiveTimelines();
      currentSection = 'skills';
      continue;
    } else if (isShortLine && /^(awards|certifications|certificates|achievements|recognition|recognitions|honors|accomplishments|awards:)/i.test(line)) {
      flushActiveTimelines();
      currentSection = 'certificates';
      continue;
    } else if (isShortLine && /^(summary|professional summary|profile|about me|professional profile|summary:)/i.test(line)) {
      flushActiveTimelines();
      currentSection = 'summary';
      parsed.personal!.bio = ''; // Clear pre-header guessed bio to capture a dedicated summary section
      continue;
    } else if (isShortLine && /^(additional information|projects|personal projects|portfolio|contact|links|socials)/i.test(line)) {
      flushActiveTimelines();
      currentSection = 'none';
      continue;
    }
    ...
    const isBulletLine = /^[•\-\*▪◦⁃‣✓✔★]/.test(line) || /^\d+\.\s/.test(line) || /^o\s/.test(line);
    const isJobTitleLine = /engineer|developer|analyst|manager|architect|designer|specialist|lead|support|consultant/i.test(line) || line.includes('|');
    const isNotAwardsOrCert = !/^(awards|certifications|certificates|projects|skills|languages|tools)/i.test(line);
    if (hasYearDate && currentSection !== 'education' && currentSection !== 'experience' && !isBulletLine && line.length < 120 && isJobTitleLine && isNotAwardsOrCert) {
      flushActiveTimelines();
      currentSection = 'experience';
    ...
    if (currentSection === 'experience') {
      ...
      if (hasDate && !isBullet && line.length < 120) {
        ...
        const period = line.match(/((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]* \d{4}|(?:19|20)\d{2})\s*[-–—]\s*(Present|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]* \d{4}|(?:19|20)\d{2})/i)?.[0] || 
                       line.match(/\b(19|20)\d{2}\b/g)?.join(' - ') || "Present";
        ...
        // Classify parts
        const positionKeywords = /engineer|developer|analyst|manager|architect|designer|specialist|lead|support|officer|consultant|expert|head/i;
        ...
        const companyKeywords = /gmbh|inc|corp|co\b|limited|ltd|llc|labs|solutions|systems|technologies|dxc/i;
        let companyIdx = cleanParts.findIndex((p, idx) => idx !== positionIdx && companyKeywords.test(p));
        if (companyIdx === -1) {
          companyIdx = cleanParts.findIndex((p, idx) => idx !== positionIdx && !/remote|berlin|bangalore|london|york|california|san/i.test(p));
        }
        if (companyIdx === -1) {
        ...
        const locationParts = cleanParts.filter((_, idx) => idx !== positionIdx && idx !== companyIdx);
        ...
        const prevLineEndedWithPeriod = i > 0 && (lines[i-1].trim().endsWith('.') || lines[i-1].trim().endsWith('!') || lines[i-1].trim().endsWith('?'));
        const isHeaderAbove = i > 0 && (lines[i-1].includes('|') || /\b(19|20)\d{2}\b/i.test(lines[i-1]) || /present/i.test(lines[i-1]));
        const isNewAchievement = isBullet || (startsWithCapital && (prevLineEndedWithPeriod || isHeaderAbove || lines[i-1].trim() === ''));
      ...
      const hasDegree = /degree|bachelor|master|phd|b\.s\.|b\.a\.|m\.s\.|bs|ba|ms|university|college|school|institute|technology|academy/i.test(line);
      ...
      if (cleanLine.length > 5) {
        ...
        let textWithoutDate = cleanLine.replace(/\s*\(\s*(19|20)\d{2}\s*\)/, '').replace(/\s*[,]?\s*\b(19|20)\d{2}\b/, '').trim();
        ...
        if (!splitDone) {
          if (/microsoft|aws|amazon|google|cisco|oracle|scrum|pmi|comptia|coursera|udemy|edx|freecodecamp/i.test(cleanLine)) {
            const matches = cleanLine.match(/(microsoft|aws|amazon|google|cisco|oracle|scrum|pmi|comptia|coursera|udemy|edx|freecodecamp)/i);
            if (matches) {
